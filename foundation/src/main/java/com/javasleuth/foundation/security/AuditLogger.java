...
public class AuditLogger implements AutoCloseable {
    ...
    private void initializeWriters() {
        if (!isAuditFileEnabled()) {
            return;
        }
        try {
            String auditPath = resolveLogPath(SleuthConfigSchema.LOGGING_AUDIT_FILE_PATH.read(config), "sleuth-audit.log");
            String securityPath = resolveLogPath(SleuthConfigSchema.LOGGING_SECURITY_FILE_PATH.read(config), "sleuth-security.log");
    ...
    private String resolveLogPath(String configured, String baseName) {
        if (configured != null && !configured.trim().isEmpty()) {
            return configured.trim();
        }
        return logPathResolver.defaultLogPath(baseName);
    }

    private boolean isAuditLoggingEnabled() {
        return SleuthConfigSchema.SECURITY_AUDIT_LOGGING.read(config);
    }

    private boolean isAuditFileEnabled() {
        return SleuthConfigSchema.LOGGING_AUDIT_ENABLED.read(config);
    }

    private boolean isAuditConsoleEnabled() {
        return SleuthConfigSchema.LOGGING_AUDIT_CONSOLE_ENABLED.read(config);
    }
    ...
    private String getAuditLogFilePath() {
        return resolveLogPath(SleuthConfigSchema.LOGGING_AUDIT_FILE_PATH.read(config), "sleuth-audit.log");
    }

    private String getSecurityLogFilePath() {
        return resolveLogPath(SleuthConfigSchema.LOGGING_SECURITY_FILE_PATH.read(config), "sleuth-security.log");
    }
    ...
    public String getAuditStatus() {
        ...
        status.append("File Output: ").append(isAuditFileEnabled() ? "ENABLED" : "DISABLED").append("\n");
        status.append("Console Output: ").append(isAuditConsoleEnabled() ? "ENABLED" : "DISABLED").append("\n");
        ...
        status.append("Audit Log File: ").append(getAuditLogFilePath()).append("\n");
        status.append("Security Log File: ").append(getSecurityLogFilePath()).append("\n");
    ...
    public String getRecentEvents(int count) {
        ...
        events.append("- ").append(getAuditLogFilePath()).append("\n");
        events.append("- ").append(getSecurityLogFilePath()).append("\n\n");
