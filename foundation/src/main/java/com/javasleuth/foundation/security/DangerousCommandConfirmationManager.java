...
public class DangerousCommandConfirmationManager {
    ...
    public ConfirmationResult confirmIfRequired(String sessionId,
        ...
        boolean requireImpactHigh = meta != null &&
            ...
            SleuthConfigSchema.SECURITY_IMPACT_HIGH_CONFIRM_ENABLED.read(config);

        if (!requireDangerous && !requireImpactHigh) {
            return ConfirmationResult.allowed(normalized);
        }

        boolean dangerousEnabled = SleuthConfigSchema.SECURITY_DANGEROUS_CONFIRM_ENABLED.read(config);
        ...
        long ttlMs = SleuthConfigSchema.SECURITY_DANGEROUS_CONFIRM_TTL_MS.read(config);
        if (ttlMs <= 0) {
            ttlMs = 60000;
        }
    ...
    private void pruneIfNeeded(long now) {
        int max = SleuthConfigSchema.SECURITY_DANGEROUS_CONFIRM_CACHE_SIZE.read(config);
    ...
    private Pending issueToken(long now, long ttlMs) {
        int bytes = SleuthConfigSchema.SECURITY_DANGEROUS_CONFIRM_TOKEN_BYTES.read(config);
