...
- `bootstrap/`：**Bootstrap 可见桥接层（spy/bridge，JDK-only）**（拦截器/数据模型/轻量工具），包根 `com.javasleuth.bootstrap.*`
- `foundation/`：非 bootstrap 可见的基础设施与通用能力（JDK-only：config/security/protocol/util 等），包根 `com.javasleuth.foundation.*`
- `agent/`：**Java Agent Bootstrap**（入口 `com.javasleuth.agent.SleuthAgent`）
  - 尽量保持最小依赖，仅负责 **引导加载 agent-core**
- `container/`：**Java Agent Container（fat-jar / composition root）**（入口 `com.javasleuth.container.SleuthAgentContainerEntrypoint`）
  - 运行在 agent 创建的隔离 ClassLoader 中，负责装配并启动 per-attach runtime
- `core/`：**Java Agent Core**（核心实现库）
  - 运行在目标 JVM 内，包含命令服务端与诊断能力的核心实现（由 container 装配启动）
...
- `container/target/java-sleuth-container-<version>-jar-with-dependencies.jar`（用于隔离加载的 runtime container）
- `launcher/target/java-sleuth-launcher-<version>-jar-with-dependencies.jar`
- `core/target/java-sleuth-agent-core-<version>.jar`（普通依赖 jar，供编译/测试/容器装配）
