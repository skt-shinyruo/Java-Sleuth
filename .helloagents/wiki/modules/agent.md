...
- Bootstrap（模块 `agent`）
  ...
  - bootstrap bridge 可用性强校验（安全前置）：
    - `SleuthAgent` 在 `appendToBootstrapClassLoaderSearch` 后 best-effort 校验关键类（如 `com.javasleuth.bootstrap.monitor.TraceInterceptor`）是否由 `BootstrapClassLoader` 加载
    - 若 bridge 不可用：打印高优先级告警并 **fail-fast 终止 agent 启动**（`premain/agentmain` 均抛异常）；避免启动命令服务与任何增强，防止业务线程触发时抛 `NoClassDefFoundError/LinkageError`
      - `agentmain`（运行时 attach）：attach 失败，但目标 JVM 不会因此崩溃
      - `premain`（`-javaagent`）：JVM 启动将失败（符合“硬前置”预期，避免带着不安全的 agent 继续启动）
