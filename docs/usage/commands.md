...
> 说明：命令服务端为 loopback-only（不支持远程暴露），并默认启用 RBAC（`security.authorization.enabled=true`）。未认证本地连接默认获得匿名 viewer 会话，可执行只读诊断命令；operator/admin/危险命令需要认证。
> 如需高权限命令，请设置 `security.auth.password.enabled=true` 和 `security.auth.*.password`（或环境变量 `SLEUTH_AUTH_*_PASSWORD`），再使用 `auth <username> <password>`；Launcher headless/restart 可用 `--auth-user <user> --auth-pass <pass>`。
...
- **备注**:
  ...
  - 口令认证默认关闭：匿名会话只能以 viewer 身份运行只读命令；需升级角色时设置 `security.auth.password.enabled=true`，并通过配置项或环境变量设置密码：
