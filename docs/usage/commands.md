...
> 说明：命令服务端为 loopback-only（不支持远程暴露），并默认启用 RBAC（`security.authorization.enabled=true`）。未认证本地连接默认不会获得匿名 viewer 会话；如需本地开发 opt-out 只读访问，可显式设置 `security.anonymous.viewer=true`。
> 如需认证访问，请设置 `security.auth.password.enabled=true` 和 `security.auth.*.password`（或环境变量 `SLEUTH_AUTH_*_PASSWORD`），再使用 `auth <username> <password>`；Launcher headless/restart 可用 `--auth-user <user> --auth-pass <pass>`。
...
- **备注**:
  ...
  - 口令认证默认关闭，匿名 viewer 也默认关闭；需认证为 viewer/operator/admin 时设置 `security.auth.password.enabled=true`，并通过配置项或环境变量设置密码：
...
> 默认启用二次确认（`security.dangerous.confirm.enabled=true`）：首次执行会返回一次性 token，需要在短 TTL 内追加 `--confirm <token>` 重试后才会真正执行。危险命令同时要求 ADMIN 权限。
...
- **用法**：
  ...
  - `vmtool track <class-pattern> [--loader <id>|--loader-id <id>|--loader-hash <id>] [--first|--unsafe-first] [--subclasses|--include-subclasses] [--max <n>] [--class-limit <n>]`
  - `vmtool instances <track-id> [--limit <n>] [--alive|--alive-only] [--where lhs:op:rhs]`
  - `vmtool inspect <track-id> <ref-id> [--deep <n>] [--fields <n>] [--static|--include-static]`
  - `vmtool invoke <track-id> <ref-id> <method> [args...] [--declared] [--unsafe] [--deep <n>] [--confirm <token>]`
  - `vmtool invoke-static <class-pattern> <method> [args...] [--loader <id>|--loader-id <id>|--loader-hash <id>] [--first|--unsafe-first] [--declared] [--unsafe] [--deep <n>] [--confirm <token>]`
  - `vmtool invokestatic <class-pattern> <method> [args...] [--loader <id>|--loader-id <id>|--loader-hash <id>] [--first|--unsafe-first] [--declared] [--unsafe] [--deep <n>] [--confirm <token>]`
  - `vmtool stop <track-id>`
  - `vmtool histogram <class-pattern> [--top <n>] [--all]`
- **选项范围**：
  - `track --max <n>`：`1..100000`，默认 `500`
  - `track --class-limit <n>`：`1..10000`，默认 `50`
  - `instances --limit <n>`：`1..10000`，默认 `50`
  - `inspect --deep <n>` / invoke `--deep <n>`：非负整数，默认 `1`
  - `inspect --fields <n>`：正整数，默认 `80`
  - `histogram --top <n>`：`1..200`，默认 `20`
- **筛选条件（--where）**：
  - `class/id/thread/ageMs`（元数据）
  - `field.xxx`（读取实例字段，敏感字段名自动脱敏）
  - `op`：`eq/ne/gt/gte/lt/lte/contains/startswith/endswith`
- **权限备注**：
  - `invoke` / `invoke-static` / `invokestatic` 支持 `--confirm <token>`；开启危险命令二次确认后，首次调用会返回一次性 token。
  - 启用 RBAC 时，invoke 路径需要 ADMIN 权限。
