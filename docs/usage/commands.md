...
> 说明：命令服务端为 loopback-only（不支持远程暴露），并默认关闭 RBAC（`security.authorization.enabled=false`），因此多数情况下无需认证即可使用。
> 只有当你显式启用 RBAC（`security.authorization.enabled=true`）并配合口令认证（`security.auth.password.enabled=true`）时，`auth/perm` 等权限相关能力才会真正生效（建议在多用户主机开启）。
